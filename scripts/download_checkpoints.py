...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Safe checkpoint status CLI cho submission repo. "
            "Script này không auto-download, không copy checkpoint và không tạo file checkpoint."
        )
    )
    ...
    status_parser = subparsers.add_parser(
        "status",
        help="In checkpoint status theo manifest hiện có.",
        description=(
            "Hiển thị checkpoint policy và nhắc dùng verify_artifacts.py để kiểm tra "
            "artifact local thay vì tự động tải checkpoint."
        ),
    )
...
def read_manifest(path: Path) -> tuple[list[dict[str, str]], list[str]]:
    warnings: list[str] = []
    if not path.exists():
        warnings.append(f"Thiếu checkpoints manifest: {path}")
        return [], warnings
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = [dict(row) for row in csv.DictReader(handle)]
    if not rows:
        warnings.append(f"Checkpoints manifest rỗng: {path}")
...
def run_status(args: argparse.Namespace) -> int:
    ...
    if r013_row is None:
        runtime_warnings.append("Không tìm thấy row R013_REPRO trong checkpoints manifest.")

    print("checkpoint_status_entrypoint")
...
def main() -> int:
    ...
    if not hasattr(args, "handler"):
        parser.error("Thiếu subcommand. Dùng `status` để xem checkpoint status.")
